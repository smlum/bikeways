#!/usr/bin/env python3
"""One-off: seed checklist.csv's prior_url from the 2024 CCND vintage.
...
Only fills prior_url where the checklist row's prior_url is still blank,
...
prior_url is permanent reference data, distinct from source_url (this
review cycle's confirmed judgment) — the checklist UI pre-fills from it as
a lead but never overwrites it, so a mistaken "Not found" doesn't destroy
the historical link. Every CCND URL is 1-3 years old and may be dead or
superseded, so treat these as leads to verify, not confirmed sources.
...
def main():
    ...
    if "prior_url" not in checklist.columns:
        checklist["prior_url"] = ""
    ...
    for _, ccnd_row in ccnd.iterrows():
        ...
        open_row = matches[checklist.loc[matches, "prior_url"] == ""]
        if len(open_row) > 0:
            idx = open_row[0]
        else:
            # Every existing row already has a prior_url — duplicate one for this lead.
            idx = len(checklist)
            checklist.loc[idx] = checklist.loc[matches[0]]
            checklist.at[idx, "prior_url"] = ""
            checklist.at[idx, "source_url"] = ""
            checklist.at[idx, "source_ids"] = ""
            checklist.at[idx, "checked_date"] = ""
            checklist.at[idx, "checked"] = ""
            duplicated.append(provider_id)

        checklist.at[idx, "prior_url"] = ccnd_row["source_url"]
    ...
    print(f"Filled prior_url for {len(matched)} row(s)")
    print(f"{len(duplicated)} required a duplicated row (provider already had a prior_url)")
